    ...
    <div class="admin-wrapper">
        ...
        <section class="admin-table--wrapper">
            <div class="admin-table--manage">
                <a href="<?=BASE_URL.'admin/overview/create.php';?>" class="admin-table--manage_btn btn btn-warning">Добавить рецензию</a>
                <a href="<?=BASE_URL.'admin/overview/index.php';?>" class="admin-table--manage_btn btn btn-success">Редактирование</a>
            </div>
            <table class="admin-table overview-table table align-middle">
                <thead>
                <tr>
                    <th scope="col">ID</th>
                ...
                <tbody>
                <?php foreach ( $overviewAdm as $key=> $film):?>
                    <tr>
                        <th scope="row"><?=$key+1;?></th>
                        <td><?=$film['title'];?></td>
                        <td class="admin-table-control">
                            <a class="admin-table-control_btn btn-primary" href="edit.php?edit_id=<?=$film['id'];?>">edit</a>
                            <a class="admin-table-control_btn btn-danger" href="index.php?delete_id=<?=$film['id'];?>">delete</a>
                        </td>
                        <?php if($film['status']):?>
                            <td class="status"><a href="edit.php?publish=0&pub_id=<?=$film['id'];?>">unpublish</a></td>
                        <?php else:?>
                            <td class="status"><a href="edit.php?publish=1&pub_id=<?=$film['id'];?>">publish</a></td>
                        <?php endif; ?>
                    </tr>
                <?php endforeach; ?>
                </tbody>
